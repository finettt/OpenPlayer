...
## Combat
- **attack_entity(target, type_filter?)** — Attack a nearby entity (mob or player). Target selectors: "nearest" (closest entity), "nearest_hostile", "nearest_passive", an entity name (e.g. "zombie", "skeleton", "blaze", "enderman", "cow"), or a player username. Use type_filter with "nearest" to narrow to "hostile", "passive", or "mob". The bot pursues and attacks with the best available weapon (or bare hand) until the target dies, you call attack_entity again, or you stop defense_mode. Returns an error if no matching entity is found or target is out of range (>48m).
- **bow_attack(target, type_filter?, charge_ticks?)** — Shoot an arrow at a target using a bow. Same target selectors as attack_entity. Automatically equips the best bow, draws it for full charge (~1 sec, configurable via charge_ticks), calculates aim with gravity compensation and target leading, then fires. Requires a bow and at least 1 arrow in inventory. Effective range ~55 blocks. Follow up with attack_entity or defense_mode for melee.
- **defense_mode(enabled)** — Toggle automatic self-defense mode. When enabled (true), the bot continuously fights hostile mobs within 16 blocks using situation-aware tactics: melee, bow for distant targets (>8 blocks, if bow+arrows available), swarm-pillar, swarm-kite, and retreat. Auto-selects the best weapon per target type, equips shield, crit-jumps, and fires arrows with gravity compensation and leading. When disabled (false), stops auto-attacking. Use this for sustained combat without manual calls.
...
- **consume(item)** — Consume an edible or drinkable item from inventory — eat food to restore hunger, drink potions for effects, or use milk to clear effects. Use "best_food" as the item name to automatically pick the best available food for your current hunger level. Partial names work (e.g. "beef" matches cooked_beef). Returns updated health, hunger, saturation, and any active effects after consumption. Prevents eating regular food when hunger is full (golden apples, chorus fruit, and potions can still be consumed at full hunger).
